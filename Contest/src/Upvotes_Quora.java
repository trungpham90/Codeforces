...
public class Upvotes_Quora {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int i = 0; i < n; i++) {
			appendRight(true, i, data[i], increase);
			appendRight(false, i, data[i], decrease);
			if (i >= k) {
				removeLeft(true, i - k, increase);
				removeLeft(false, i - k, decrease);
			}
			if (i + 1 >= k) {
				// System.out.println(i + " " + increase.total + " "
				// + decrease.total);
				out.println(increase.total - decrease.total);
			}
		}
		out.close();
	}

	static void appendRight(boolean increase, int index, int val, Node node) {
		...
		appendRight(increase, index, val, node.left);
		appendRight(increase, index, val, node.right);
		...
		if (node.left.a == -1 && node.right.b == -1) {
			...
			if (node.left.isContinuos) {
				if (isContinuous(increase, node.left.b, node.right.a)) {
					node.leftSide = node.left.leftSide + node.right.rightSide;
				}
			} else {
				node.leftSide = node.left.leftSide;
			}
	...
	static void removeLeft(boolean increase, int index, Node node) {
		if (node.l > index || node.r < index) {
			return;
		}
		if (node.l == index && node.r == index) {
		...
		removeLeft(increase, index, node.left);
		removeLeft(increase, index, node.right);
		if (node.left.a == -1 && node.right.b == -1) {
			node.total = 0;
			node.a = -1;
			node.b = -1;
